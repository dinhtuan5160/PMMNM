...
class SanPhamController extends Controller
{
    ...
    public function getSua($id){
    	...
    	return view('admin.sanpham.sua',compact('theloai','sanpham'));
    }

    public function postSua(Request $request,$id){
    	$loaisanpham = LoaiSanPham::find($id);
    	$this->validate($request,[
                'ten'=>'required|min:3|max:100|unique:LoaiSanPham,ten',
                'theloai'=>'required',
                'ghichu'=>'required' 
            ],
            [
                ...
                'ten.min'=>'Tên loại sản phẩm phải có độ dài từ 3 đến 100 kí tự',
                'ten.max'=>'Tên loại sản phẩm phải có độ dài từ 3 đến 100 kí tự',
        ]);

        $loaisanpham->ten = $request->ten;
        $loaisanpham->tenkhongdau = str_slug($request->ten,' ');
        $loaisanpham->id_theloai = $request->theloai;
        $loaisanpham->ghichu = $request->ghichu;
